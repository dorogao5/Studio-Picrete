...
export default function BankPlayground({ assistant }: { assistant: Assistant }) {
  ...
  return <div className="space-y-5">
    ...
    <Card className="p-5 space-y-2 text-sm"><p>После ошибки уточните «Нюансы» профиля или создайте и активируйте новую версию промпта «Проверка решений». Повторите сохранённые ответы и отметьте корректную проверку.</p><p>Затем откройте «Курсы» → «Опубликовать обновление». Публикация переносит профиль, активные промпты и справочники. Новые задачи из банка получают ту же шкалу; уже созданные работы сохраняют свои критерии.</p><Link className="text-accent underline" to={`/assistants/${assistant.id}?tab=assistant`}>Открыть настройки и публикацию ассистента →</Link></Card>
  </div>;
}
