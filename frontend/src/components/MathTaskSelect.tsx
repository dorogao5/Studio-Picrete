...
export function MathTaskSelect({ value, onChange, options, placeholder, disabled = false, searchLabel = "Поиск задачи", allowClear = true }: {
  value: string; onChange: (value: string) => void; options: { id: string; text: string }[]; placeholder: string; disabled?: boolean; searchLabel?: string; allowClear?: boolean;
}) {
  ...
  return <div className="relative min-w-0" onBlur={event => { if (!event.currentTarget.contains(event.relatedTarget)) setOpen(false); }} onKeyDown={event => { if (event.key === "Escape") setOpen(false); }}>
    ...
    {open && <div id={id} className="absolute z-40 mt-1 w-full rounded-md border border-border bg-card p-2 shadow-lg">
      <input aria-label={searchLabel} value={query} onChange={event => setQuery(event.target.value)} placeholder={searchLabel} className="mb-2 w-full rounded border border-input bg-background p-2 text-sm" />
      <div className="max-h-72 overflow-y-auto">
        {allowClear && <button type="button" className="w-full rounded p-2 text-left text-sm hover:bg-muted" onClick={() => choose("")}>{placeholder}</button>}
        ...
        {!matches.length && <p className="p-2 text-sm text-muted-foreground">Ничего не найдено</p>}
      </div>
    </div>}
  </div>;
}
