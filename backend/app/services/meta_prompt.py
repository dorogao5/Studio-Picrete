...
def build_meta_prompt(assistant: Assistant, role: str, target_family: str, extra_instructions: str = "") -> str:
    if role == "grader":
        role_block = (
            ...
            "Эталон преподавателя считается верным: не предлагайте искать его дефекты или проверять как гипотезу. "
            "Принимайте корректные эквивалентные методы, если условие не предписывает конкретный способ, "
            "а также эквивалентные обозначения, единицы и допустимое округление. "
            "Не требуйте буквального повторения эталона, порядка шагов или одинаковой подробности. "
            "Краткость не является ошибкой, если записанного достаточно для проверки условия и рубрики; "
            "явно требуемые обоснования и промежуточные результаты должны присутствовать. "
            "Каждое снижение баллов обосновывайте фактической ошибкой или невыполненным требованием. "
            "Не приписывайте отсутствующие шаги и не штрафуйте одну ошибку повторно. "
            "Существенную неоднозначность OCR передавайте преподавателю без догадок; "
            "альтернативный метод сам по себе не требует такой передачи. "
        ...
        role_block = (
            ...
            "Каждая задача должна включать ясное самодостаточное условие с данными, единицами, "
            "необходимыми допущениями и явным перечнем вопросов; последовательное эталонное решение; "
            "полный ответ на все вопросы; согласованную рубрику оценивания. "
            "Проверьте достаточность данных, вычисления и согласованность обозначений и результатов "
            "между условием, решением и ответом. Сумма максимумов рубрики должна равняться max_score. "
            "Рубрика проверяет требования условия, принимает эквивалентные методы и разную подробность "
            "решения, предусматривает частичный балл и не штрафует одну ошибку повторно.\n\n"
