...
ESSENTIAL_TOOLS_INSTRUCTION = (
    ...
    "числа, не символы. В calculator заменяйте π числовым литералом 3.141592653589793 "
    "(его точности достаточно для задач курса), а экспоненту записывайте как exp(x); "
    "в учебных формулах сохраняйте π. Для выражений с символами используйте sympy с явно объявленными symbols; "
    ...
    'У reference_db начинайте с query (формула вещества предпочтительна) и property; ID не угадывайте. Пример: query="AgCl", property="solubility_product". Для термодинамики задавайте нужную phase; temperature_k указывайте только если требуется именно эта температура. При match_status=unique используйте normalized_result.record. При ambiguous смотрите candidates и уточняйте свойство, фазу или реакцию; не выбирайте первый результат без проверки. При conditions_unverified условия не подтверждены; не приписывайте записи запрошенную температуру или фазу. При not_found данных по этому запросу нет: не выдумывайте ID и числа; уточните формулу/свойство, запросите исходные данные или выберите обеспеченный данными вариант генерируемой задачи. Если next_offset не null, доступны остальные кандидаты через offset; лучше уточнить запрос. Успех поиска означает выполнение поиска, а не наличие подходящих данных. Поиск уже возвращает значения, поэтому повторный lookup по reference_id не нужен, если запись полностью получена. Используйте value, unit, conditions, source выбранной записи; учитывайте uncertainty_or_warning и review_warnings. Не смешивайте фазы, стандартные и нестандартные условия; не объявляйте отсутствующие единицы известными. Численные расчёты с полученными значениями выполняйте calculator. При генерации сначала получите необходимые справочные данные, затем задавайте согласованные условия задачи и решайте её. При проверке используйте условия исходной задачи, не меняйте их ради найденной записи. Не сериализуйте весь record в ответ, если запрошено только значение. '
